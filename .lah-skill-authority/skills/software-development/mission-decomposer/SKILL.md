...
| **Script échoue pour READ_ONLY_AUDIT / DESIGN_ONLY** | `decompose-mission.mjs` rencontre exactement `Phase gate6_tests depends on unknown phase: gate5b_implementation` (le template de phases est calé sur CODE_CHANGE) | Le script retourne déterministiquement `DECOMPOSER_FALLBACK_READ_ONLY` + `SATISFIED_BY_DOCUMENTED_FALLBACK`; Gate 0.5 continue en exécution directe. Toute autre erreur reste `DECOMPOSER_FAILURE_BLOCKED` et fail-closed |
